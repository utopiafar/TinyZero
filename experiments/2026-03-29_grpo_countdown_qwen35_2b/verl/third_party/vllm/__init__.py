...
if package_version == '0.3.1':
    ...
    # For newer vLLM versions (e.g., 0.18.x), try the latest adapter.
    # This allows HF rollout to work without vLLM-specific features.
    import warnings
    warnings.warn(
        f'vllm version {package_version} not officially supported. '
        f'Falling back to 0.6.3 adapter. Currently supported versions are 0.3.1, 0.4.2, 0.5.4 and 0.6.3.',
        RuntimeWarning,
    )
    vllm_version = '0.6.3'
    try:
        ...
        # If even the fallback fails, create dummy stubs for HF rollout mode
        LLM = None
        LLMEngine = None
        parallel_state = None
