...
class Tracking(object):
    ...
    def __init__(self, project_name, experiment_name, default_backend: Union[str, List[str]] = 'console', config=None):
        ...
        if 'tensorboard' in default_backend:
            ...
            import os
            tb_log_dir = os.environ.get('TB_LOG_DIR', os.path.join(os.path.expanduser('~'), 'tf-logs'))
            tb_log_dir = os.path.join(tb_log_dir, 'training_metrics')
            os.makedirs(tb_log_dir, exist_ok=True)
            self._tb_writer = SummaryWriter(log_dir=tb_log_dir)
            self.logger['tensorboard'] = _TensorBoardAdapter(self._tb_writer)
